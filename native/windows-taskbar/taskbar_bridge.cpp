...
constexpr int kInstallBlockedByIntegrity = 2;
...
bool ProcessHasHighIntegrity(HANDLE process, bool* result) {
    ...
    const bool queried = bytes > 0 && GetTokenInformation(token, TokenIntegrityLevel, buffer.data(), bytes, &bytes);
    CloseHandle(token);
    ...
    *result = *GetSidSubAuthority(label->Label.Sid, count - 1) >= SECURITY_MANDATORY_HIGH_RID;
    return true;
}

// Explorer owns the thumbnail-toolbar UI. UIPI prevents a normal Explorer process from sending
// WM_COMMAND to an elevated app, so avoid registering controls that are guaranteed not to work.
bool ExplorerBlocksTaskbarCommands() {
    bool currentHigh = false;
    if (!ProcessHasHighIntegrity(GetCurrentProcess(), &currentHigh) || !currentHigh) return false;
    const HWND shell = GetShellWindow();
    DWORD explorerPid = 0;
    if (shell == nullptr || GetWindowThreadProcessId(shell, &explorerPid) == 0) return false;
    HANDLE explorer = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, explorerPid);
    if (explorer == nullptr) return false;
    bool explorerHigh = false;
    const bool queried = ProcessHasHighIntegrity(explorer, &explorerHigh);
    CloseHandle(explorer);
    return queried && !explorerHigh;
}
...
extern "C" __declspec(dllexport) int __stdcall lazer_taskbar_install(HWND hwnd,
    ...
    if (ExplorerBlocksTaskbarCommands()) {
        DebugLog(L"install-blocked-integrity", WindowDetails(hwnd));
        return kInstallBlockedByIntegrity;
    }
